...
app.get('/api/admin/stats', verifyAdminToken, asyncHandler(async (req, res) => {
    ...
    if (USE_POSTGRES) {
        ...
        totalRevenue = await db.query("SELECT COALESCE(SUM(total), 0) FROM orders WHERE payment_status = 'paid'");
        ...
        totalRevenue = { rows: [{ sum: db.prepare("SELECT COALESCE(SUM(total), 0) as sum FROM orders WHERE payment_status = 'paid'").get().sum }] };
